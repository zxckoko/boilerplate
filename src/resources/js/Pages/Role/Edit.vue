    ...
    <AuthenticatedLayout>
        ...
        <div class="max-w-full m-8 mb-0 pb-8">
            <Form @submit="form.patch(route('roles.update', role.id))" class="flex flex-col space-y-4 max-w-1/3 my-2">
                ...
                <fieldset class="border border-2 p-2" :disabled="canEditRolePermissions">
                    <legend class="border text-xs text-gray-400 py-1 px-1 mb-2 ml-2">
                        <div class="flex items-center gap-2">
                            <span>Permissions</span><i v-if="canEditRolePermissions" class="pi pi-ban text-secondary"></i>
                        </div>
                    </legend>
            ...
            <ConfirmDeleteDialog
                v-if="role.deleted_at === null && can('roles.destroy')"
                :record="route('roles.destroy', role.id)"
            />

            <ConfirmRestoreDialog
                v-if="role.deleted_at !== null && can('restoreRecord')"
                :record="route('roles.restore', role.id)"
            />
...
const canEditRolePermissions = ! can('editRolePermissions');
